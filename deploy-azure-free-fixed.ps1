...
az webapp create --name $apiAppName --resource-group $resourceGroup --plan $appServicePlan --runtime "DOTNETCORE:8.0"
...
az staticwebapp create --name $staticAppName --resource-group $resourceGroup --location "eastus2"
...
az webapp config appsettings set --name $apiAppName --resource-group $resourceGroup --settings "ASPNETCORE_ENVIRONMENT=Production" "ConnectionStrings__DefaultConnection=$connectionString" "VITE_API_BASE_URL=https://$apiAppName.azurewebsites.net"
...
try {
    ...
    # Deploy to Azure
    az webapp deployment source config-zip --name $apiAppName --resource-group $resourceGroup --src "..\..\..\whoptix-deploy.zip"
...
try {
    ...
    "VITE_API_BASE_URL=https://$apiAppName.azurewebsites.net" | Out-File -FilePath ".env.production" -Encoding ASCII
